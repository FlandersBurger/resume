...
const adminOnly = async (game, player) => {
  ...
  if (player.infractions < 9) {
    game.provider.message(
      game,
      `${(0, i18n_1.default)(game.settings.language, "warnings.adminFunction", { name: (0, players_1.getPlayerName)(player) })}\nID: ${player.id}`,
    );
  } else if (player.infractions === 9) {
    game.provider.message(
      game,
      `${(0, i18n_1.default)(game.settings.language, "warnings.abuse", { name: (0, players_1.getPlayerName)(player) })}\nID: ${player.id}`,
    );
  } else {
    game.provider.message(
      game,
      `${(0, i18n_1.default)(game.settings.language, "warnings.banned", { name: (0, players_1.getPlayerName)(player) })}\nID: ${player.id}`,
    );
    telegram_1.default.notifyAdmin(
      `Banned player: ${(0, players_1.getPlayerName)(player)}\nID: ${player.id}\nChat: https://belgocanadian.com/tenthings/${game.chat_id}`,
    );
    player.banned = true;
  }
  await player.save();
