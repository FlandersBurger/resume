...
const processGuess = async (guess) => {
    ...
    if (guess.match.type === game_1.GameType.MAINGAME) {
        ...
        console.log(`${guess.gameId} (${game.settings.language}) - ${game.list.name} ${guess.match.value} by ${(0, players_1.getPlayerName)(player)}`);
    }
    else if (guess.match.type === game_1.GameType.MINIGAME) {
        await (0, minigame_1.checkMinigame)(game, player, guess);
        console.log(`${guess.gameId} (${game.settings.language}) - ${"Minigame"} ${game.minigame.answer} by ${(0, players_1.getPlayerName)(player)}`);
    }
    else if (guess.match.type === game_1.GameType.TINYGAME) {
        await (0, tinygame_1.checkTinygame)(game, player, guess);
        console.log(`${guess.gameId} (${game.settings.language}) - ${"Tinygame"} ${game.tinygame.answer} by ${(0, players_1.getPlayerName)(player)}`);
    }
};
