...
const sendSuggestion = async (msg, game, player, suggestionType) => {
    if (game && player) {
        ...
        if (!msg.text) {
            telegram_1.default.queueMessage(msg.chatId, (0, i18n_1.default)(game.settings.language, "warnings.noSuggestion", { name: playerName }), msg.topicId);
            ...
            telegram_1.default.queueMessage(msg.chatId, `${message}Thank you, ${playerName}`, msg.topicId);
...
const sendSuggestionMessage = async (game, player, suggestionType) => {
    ...
    switch (suggestionType) {
        case "feature":
            ...
            await player.save();
            telegram_1.default.sendMessage(game.chat_id, `<b>FEATURE</b>\nPlease suggest your feature in your next message, ${playerName}!`, { topic: game.topicId });
            break;
        case "typo":
            player.state = player_1.PlayerState.Typo;
            await player.save();
            telegram_1.default.sendMessage(game.chat_id, `<b>TYPO</b>\nPlease let me know what the typo is in your next message, ${playerName}!\nMention the list name too if the typo is not part of: <i>"${(0, string_helpers_1.parseSymbols)(game.list.name)}"</i>`, { topic: game.topicId });
            break;
        case "bug":
            player.state = player_1.PlayerState.Bug;
            await player.save();
            telegram_1.default.sendMessage(game.chat_id, `<b>BUG</b>\nPlease provide some details as to what went wrong in your next message, ${playerName}!`, { topic: game.topicId });
            break;
        default:
            break;
    }
