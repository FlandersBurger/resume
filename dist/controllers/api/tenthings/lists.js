...
exports.tenthingsListsRoute.get("/:id/report/:user", async (req, res) => {
  if (!res.locals.isAuthorized) res.sendStatus(401);
    ...
    if (!list) res.sendStatus(404);
      ...
      else telegram_1.default.notifyAdmins("Check: " + list.name + " reported by " + user.username);
    }
  }
});
    ...
    if (!list) res.sendStatus(404);
      ...
      if (!updatedList) res.sendStatus(404);
        ...
        if (previousModifyDate < yesterday && !res.locals.isAdmin) {
          telegram_1.default.notifyAdmins(
            `<u>List Updated</u>\nUpdated by <i>${res.locals.user?.username}</i>\n${telegram_2.telegram.listMessage(updatedList)}`,
            (0, keyboards_1.curateListKeyboard)(list),
          );
        }
...
exports.tenthingsListsRoute.post("/", async (req, res) => {
  if (!res.locals.isAuthorized) res.sendStatus(401);
    ...
    const list = await index_1.List.findByIdAndUpdate(
      req.body.list._id ? req.body.list._id : new mongoose_1.Types.ObjectId(),
      req.body.list,
    ...
    if (!updatedList) res.sendStatus(404);
      ...
      if (!req.body.list._id) {
        telegram_1.default.notifyAdmins(
          `<u>List Created</u>\n${telegram_2.telegram.listMessage(updatedList)}`,
          (0, keyboards_1.curateListKeyboard)(updatedList),
        );
