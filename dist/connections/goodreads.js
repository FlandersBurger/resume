...
exports.getAuthor = async (author) => {
    const goodreadsAuthor = await axios.get(`https://www.goodreads.com/api/author_url/${author}?key=${process.env.GOODREADS_TOKEN}`);
...
exports.getBookCover = async (query, author) => {
    const goodreadsDB = await axios.get(`https://www.goodreads.com/search.xml?key=${process.env.GOODREADS_TOKEN}&q=${encodeURIComponent(query)}&search[field]=title`);
    try {
