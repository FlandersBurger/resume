...
class Spotify {
    constructor(token) {
        ...
        this.getArtist = async (search) => {
            const { data } = await axios.get(`https://api.spotify.com/v1/search?q=${encodeURI(search)}&type=artist`, this.getHeaders());
            let artist = data.artists.items.filter((artist) => artist.followers.total > 500)[0];
            const { data: trackData } = await axios.get(`https://api.spotify.com/v1/artists/${artist.id}/top-tracks?market=US`, this.getHeaders());
            artist.tracks = trackData.tracks.map((track) => ({
                name: track.name,
                url: track.preview_url,
            }));
            const { data: albumData } = await axios.get(`https://api.spotify.com/v1/artists/${artist.id}/albums?include_groups=album&limit=50`, this.getHeaders());
