...
function Asteroids() {
    ...
    const [announce, setAnnounce] = (0, react_1.useState)(false);
    const [announcement, setAnnouncement] = (0, react_1.useState)("");
    const [spaceIndex, setSpaceIndex] = (0, react_1.useState)(0);
    const gameRef = (0, react_1.useRef)({
        playing: false,
        score: 0,
        highscore: 0,
        shots: {},
    ...
    (0, react_1.useEffect)(() => {
            ...
            },
            {
                ...
                activate: (_) => {
                    for (const i in g.asteroids)
                        g.asteroids[i].explode();
                    spawnAsteroids(5);
                },
            },
        ];
        powerupTypes.forEach((p) => {
            p.img.src = `asteroids/${p.name}.png`;
        });
        ...
        function hit(o1, o2) {
            return (o1.position[0] < o2.position[0] + o2.width &&
        ...
        function moveObject(obj) {
            ...
            if (obj.position[0] > canvas.width)
                obj.position[0] = -obj.width;
            else if (obj.position[0] < -obj.width)
                obj.position[0] = canvas.width;
            if (obj.position[1] > canvas.height)
                obj.position[1] = -obj.height;
            else if (obj.position[1] < -obj.height)
                obj.position[1] = canvas.height;
            ctx.save();
            ...
            if (obj.cycle) {
                ...
                ctx.drawImage(obj.img, obj.cycle.size[0] * column, obj.cycle.size[1] * row, obj.cycle.size[0], obj.cycle.size[1], -obj.width / 2, -obj.height / 2, obj.width, obj.height);
                if (obj.cycle.i <= 0) {
                    obj.cycle.i = 0;
                    obj.cycle.direction = true;
                }
        ...
        function makeShot(id, spaceship) {
            const s = {
                id,
                width: 9,
                height: 15,
        ...
        function makeAsteroid(id) {
            ...
            a.move = () => {
                ...
                if (a.rotation > 360)
                    a.rotation -= 360;
                else if (a.rotation < 0)
                    a.rotation = 360 + a.rotation;
                for (const i in g.shots) {
                    if (hit(g.shots[i], a)) {
                        a.explode();
                        const points = Math.floor((g.shots[i].speed - 500) / 10 + 70 - a.width);
                        if (Math.floor((points + g.score) / 100) > Math.floor(g.score / 100))
                            spawnPowerup();
                        g.score += points;
                        g.setScore(g.score);
                        delete g.shots[i];
        ...
        function makeExplosion(obj) {
            ...
            e.move = () => {
                e.lifespan--;
                moveObject(e);
                e.cycle.direction ? e.cycle.i++ : e.cycle.i--;
            };
            return e;
        }
        function makePowerup(id) {
            ...
            if (cyc.size[1] > cyc.size[0]) {
                p.height = 40;
                p.width = Math.round((p.height / cyc.size[1]) * cyc.size[0]);
            }
            else {
                p.width = 40;
                p.height = Math.round((p.width / cyc.size[0]) * cyc.size[1]);
            }
            ...
            p.move = () => {
                ...
                moveObject(p);
                if (delay <= 0) {
                    p.cycle.direction ? p.cycle.i++ : p.cycle.i--;
                    delay = 5;
                }
                else
                    delay--;
        ...
        function autoSpawn() {
            ...
            if (g.playing)
                g.spawnTimeout = setTimeout(autoSpawn, spawnInterval());
        }
        function gameOver() {
            g.playing = false;
            g.setPlaying(false);
        ...
        function evaluateKeys() {
            const s = g.spaceship;
            if (g.playing && s) {
                if (g.map[32] && s.cooldown === 0)
                    s.shoot();
                if (g.map[37])
                    s.rotation = s.rotation === 0 ? 360 : s.rotation - 3;
                if (g.map[39])
                    s.rotation = s.rotation === 360 ? 0 : s.rotation + 3;
                if (g.map[38]) {
        ...
        function draw() {
            ...
            for (const i in g.asteroids)
                g.asteroids[i].move();
            for (const i in g.powerups)
                g.powerups[i].move();
            for (const i in g.explosions) {
                if (g.explosions[i].lifespan <= 0)
                    delete g.explosions[i];
                else
                    g.explosions[i].move();
            }
            evaluateKeys();
            if (g.playing) {
                ...
                const t = g.tally;
                const c = (a, b) => Math.floor(a + (b * t) / 100);
        ...
        const onKeyDown = (e) => {
            g.map[e.keyCode || e.which] = true;
        };
        const onKeyUp = (e) => {
            g.map[e.keyCode || e.which] = false;
        };
        resizeCanvas();
