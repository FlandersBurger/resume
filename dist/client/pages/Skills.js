...
function DoughnutChart({ percentage, image }) {
    ...
    return (<div className="selected-skill" style={{ position: "relative", width: 150, height: 150, margin: "0 auto" }}>
      <canvas ref={canvasRef}/>
      <img src={`/skills/${image}`} alt="" style={{
            position: "absolute",
            top: "50%",
            left: "50%",
...
function Skills() {
    ...
    (0, react_1.useEffect)(() => {
        fetch("/skills.json")
            ...
            const cats = enabled.reduce((acc, skill) => {
            ...
            setCategories(cats);
            if (enabled.length)
                setSelectedSkill(enabled[0]);
        });
        fetch("/experience.json")
            .then((r) => r.json())
            .then((jobs) => {
            setProjects(jobs.reduce((acc, j) => (j.projects ? acc.concat(j.projects) : acc), []));
        });
    }, []);
    ...
    return (<div id="skills-page">
      ...
      <div className="skill-selector-wrap col-xs-3" style={{ padding: 0 }}>
        <div className="skill-selector">
          {Object.entries(categories).map(([categoryName, categorySkills]) => (<div key={categoryName}>
              <h3 className="hidden-xs">{categoryName}</h3>
              {[...categorySkills]
                .sort((a, b) => b.percentage - a.percentage)
                .map((skill) => (<div key={skill.code} className={`selectable-skill${selectedSkill?.code === skill.code ? " highlighted-skill" : ""}`} onClick={() => setSelectedSkill(skill)}>
      ...
      <div className="skill-display col-xs-9">
        {selectedSkill && (<>
            ...
            <div className="selected-skill-text">
              ...
              <p dangerouslySetInnerHTML={{ __html: dompurify_1.default.sanitize(selectedSkill.description) }}/>
              {selectedSkill.tldr && (<p>
                  <strong title="Too Long; Didn't Read">TL;DR</strong>{" "}
                  <span dangerouslySetInnerHTML={{ __html: dompurify_1.default.sanitize(selectedSkill.tldr) }}/>
                </p>)}
              {filteredProjects.length > 0 && (<>
                  <h3>Used in projects:</h3>
                  <ul>
                    {filteredProjects.map((p) => (<li key={p.name}>{p.name}</li>))}
                  </ul>
                </>)}
            </div>
          </>)}
