...
function Contact() {
    const { toast } = (0, AppContext_1.useApp)();
    const [sent, setSent] = (0, react_1.useState)(false);
    const [sending, setSending] = (0, react_1.useState)(false);
    const [contact, setContact] = (0, react_1.useState)({ email: "", name: "", phone: "", about: "", message: "" });
    const handleSend = async (e) => {
        e.preventDefault();
        setSending(true);
        try {
            await (0, email_1.sendEmail)(contact);
            toast("Message sent");
            setSent(true);
        }
        catch {
    ...
    return (<div id="contact-page">
      ...
      {!sent && (<form className="form-horizontal" onSubmit={handleSend}>
          ...
          <div className="form-group">
            ...
            <div className="col-sm-10">
              <input className="form-control" type="tel" value={contact.phone} onChange={(e) => setContact({ ...contact, phone: e.target.value })}/>
            </div>
          </div>
          <div className="form-group">
            ...
            <div className="col-sm-10">
              <div className="btn-group btn-group-vertical" style={{ width: "100%" }}>
                {OPTIONS.map((option) => (<button key={option.name} type="button" className={`btn col-sm-12 ${contact.about === option.name ? "btn-primary" : "btn-default"}`} style={{ marginBottom: 4, whiteSpace: "normal" }} onClick={() => setContact({ ...contact, about: option.name })}>
                    {option.text}
                  </button>))}
              </div>
            </div>
          ...
          <div className="form-group">
            ...
            <div className="col-sm-10">
              <textarea className="form-control" rows={5} id="comment" value={contact.message} onChange={(e) => setContact({ ...contact, message: e.target.value })}/>
            </div>
          </div>
          <div className="form-group">
