...
function BlurbModal({ item, onClose }) {
    return (<div className="modal fade in" style={{ display: "block", backgroundColor: "rgba(0,0,0,0.5)", overflowY: "auto" }} onClick={onClose}>
      <div className="modal-dialog" onClick={(e) => e.stopPropagation()}>
        <div className="modal-content">
          ...
          <div className="modal-body" style={{ maxHeight: "calc(100vh - 200px)", overflowY: "auto" }}>
            {item.blurbType === "image" && (<img src={item.blurb} alt={item.value} style={{ maxWidth: "100%", display: "block", margin: "0 auto" }}/>)}
            {(item.blurbType === "youtube" || item.blurbType === "spotify" || item.blurbType === "link") &&
            item.blurb && (<iframe width="100%" height="315" frameBorder={0} src={getEmbedUrl(item.blurb)} allowFullScreen/>)}
          </div>
...
function AutoTextarea({ value, onChange, onBlur, placeholder, disabled, }) {
    const ref = (0, react_1.useRef)(null);
    const COLLAPSED = 43;
    const expand = () => {
        if (ref.current) {
            const natural = ref.current.scrollHeight;
            if (natural > COLLAPSED)
                ref.current.style.height = natural + "px";
        }
    };
    const collapse = () => {
        if (ref.current)
            ref.current.style.height = COLLAPSED + "px";
    };
    return (<textarea ref={ref} className="form-control" value={value} placeholder={placeholder} disabled={disabled} onChange={(e) => {
            onChange?.(e.target.value);
            expand();
        }} onFocus={expand} onBlur={() => {
            collapse();
            onBlur?.();
        }} style={{ resize: "none", overflow: "hidden", height: COLLAPSED }} maxLength={3500}/>);
}
const ValueRow = (0, react_1.memo)(function ValueRow({ v, index, readOnly, total, onUpdate, onRemove, onBlurbClick, }) {
    const [localValue, setLocalValue] = (0, react_1.useState)(v.value);
    const [localBlurb, setLocalBlurb] = (0, react_1.useState)(v.blurb ?? "");
    const prevId = (0, react_1.useRef)(v._id);
    ...
    return (<tr>
        ...
        <td className="input-cell">
          {readOnly ? (<span>{v.value}</span>) : (<AutoTextarea value={localValue} placeholder={`Answer ${total - index}`} onChange={setLocalValue} onBlur={() => {
                if (localValue !== v.value)
                    onUpdate(index, { value: localValue });
            }}/>)}
        </td>
        <td className="input-cell">
          {readOnly ? (<span>{v.blurb ?? ""}</span>) : (<AutoTextarea value={localBlurb} placeholder={`Display after guessing ${v.value || "answer"} (Can be a URL)`} onChange={setLocalBlurb} onBlur={() => {
                if (localBlurb !== (v.blurb ?? ""))
                    onUpdate(index, { blurb: localBlurb });
            }}/>)}
        </td>
        <td className="image-cell" style={{ width: 36 }} onClick={() => v.blurb && onBlurbClick(v)}>
          {v.blurb && (<img src={v.blurbType === "image"
                ? v.blurb
                : `https://img.youtube.com/vi/${v.blurb?.match(/[?&]v=([^&]+)/)?.[1] ?? ""}/default.jpg`} alt="" style={{ width: 28, height: 28, objectFit: "cover", cursor: "pointer" }} onError={(e) => {
                e.target.style.display = "none";
            }}/>)}
        </td>
        <td className={canDelete ? "delete-column" : "disabled-column"} onClick={() => canDelete && !readOnly && onRemove(index)}>
          <i className="fas fa-trash"/>
        </td>
      </tr>);
}, (prev, next) => prev.v._id === next.v._id &&
...
function ListEditor({ list, active, isAdmin, readOnly, languageOptions, categoryOptions, onClose, onChange, onBlur, onDelete, }) {
    const [newItem, setNewItem] = (0, react_1.useState)({ value: "", blurb: "" });
    ...
    const [gettingBlurbs, setGettingBlurbs] = (0, react_1.useState)(false);
    const [showLanguages, setShowLanguages] = (0, react_1.useState)(false);
    const langRef = (0, react_1.useRef)(null);
    const catRef = (0, react_1.useRef)(null);
    (0, react_1.useEffect)(() => {
        const handler = (e) => {
    ...
    const updateValue = (0, react_1.useCallback)((index, patch) => {
        const l = listRef.current;
        const updated = { ...l, values: l.values.map((v, i) => (i === index ? { ...v, ...patch } : v)) };
        onChangeRef.current(updated);
        onBlurRef.current(updated);
    }, []);
    const removeValue = (0, react_1.useCallback)((index) => {
        const l = listRef.current;
    ...
    return (<div className={`edit-list-panel${active ? " active" : ""}`}>
      ...
      <div className="input-group">
        ...
        <input className="form-control" placeholder="Name" value={list.name} disabled={readOnly} onChange={(e) => onChange({ ...list, name: e.target.value })} onBlur={() => onBlur(listRef.current)} autoComplete="off"/>
      </div>

      
      <div className="input-group">
        <span className="input-group-addon">Description</span>
        <AutoTextarea value={list.description ?? ""} placeholder="Description displays when the list is introduced to the players." disabled={readOnly} onChange={(v) => onChange({ ...list, description: v })} onBlur={() => onBlur(listRef.current)}/>
      </div>
      ...
      <div className="btn-toolbar" role="toolbar">
        ...
        <div ref={langRef} className={`dropdown btn-group${showLanguages ? " open" : ""}`}>
          <button className="btn btn-default dropdown-toggle" type="button" disabled={readOnly} title="Language" onClick={() => setShowLanguages((s) => !s)}>
            {list.language || "None"}
          </button>
          <ul className="dropdown-menu">
            {languageOptions.map((l) => (<li key={l.code} className={l.code === list.language ? "active" : ""}>
                <a href="#" onClick={(e) => {
                e.preventDefault();
        ...
        <div ref={catRef} className={`dropdown btn-group${showCategories ? " open" : ""}`}>
          <button className={`btn dropdown-toggle ${list.categories.length === 0 ? "btn-danger" : "btn-default"}`} type="button" disabled={readOnly} onClick={() => setShowCategories((s) => !s)} title="Categories">
            {getCategoryLabel()}
          </button>
          <div className="dropdown-menu" style={{ padding: "8px 12px", minWidth: 500 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {categoryOptions.map((cat) => (<div key={cat.value} style={{ display: "flex", alignItems: "flex-start", gap: 4 }}>
                  <button type="button" className={`btn btn-xs ${list.categories.includes(cat.value) ? "btn-primary" : "btn-default"}`} style={{ fontWeight: "bold", minWidth: 90, flexShrink: 0 }} onClick={() => toggleCategory(cat.value)}>
                    {cat.label}
                  </button>
                  {list.categories.includes(cat.value) && cat.subcategories && cat.subcategories.length > 0 && (<>
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 4, flex: 1 }}>
                        {cat.subcategories.map((sub) => (<button key={sub.value} type="button" className={`btn btn-xs ${list.categories.includes(sub.value) ? "btn-primary" : "btn-default"}`} onClick={() => toggleCategory(sub.value)}>
                            {sub.label}
                          </button>))}
                      </div>
                    </>)}
        ...
        <div className="btn-group" role="group">
          {difficultyIcons.map((d, i) => (<button key={i} type="button" className={`btn btn-icon ${list.difficulty === i ? "btn-primary" : "btn-default"}`} title={d.title} disabled={readOnly} onClick={() => {
        ...
        <div className="btn-group" role="group">
          {frequencyIcons.map((f, i) => (<button key={i} type="button" className={`btn btn-icon ${list.frequency === i ? "btn-primary" : "btn-default"}`} title={f.title} disabled={readOnly} onClick={() => {
        ...
        {list._id !== "new" && (<div className="btn-group" role="group">
            {BLURB_TYPES.filter((bt) => (isAdmin || !bt.adminOnly) && list.categories.some((c) => bt.categories.includes(c.split(".")[0]))).map((bt) => (<button key={bt.api} className="btn btn-default" disabled={gettingBlurbs || readOnly} onClick={async () => {
                    setGettingBlurbs(true);
                    try {
                        const updated = await (0, tenthings_1.getBlurbs)(list._id, bt.api);
                        onChange(updated);
                    }
                    finally {
      ...
      <table className="table table-striped">
        ...
        <tbody>
          ...
          {!readOnly && list.name && list.categories.length > 0 && (<tr>
              <td className="hidden-xs hidden-sm visible-md visible-lg"/>
              <td className="hidden-xs hidden-sm visible-md visible-lg"/>
              <td className="input-cell">
                <AutoTextarea value={newItem.value} placeholder={`New answer (${list.values.length + 1})`} onChange={(v) => setNewItem({ ...newItem, value: v })}/>
              </td>
              <td className="input-cell">
                <AutoTextarea value={newItem.blurb} placeholder={`Display after guessing ${newItem.value || "new answer"} (Can be a URL)`} onChange={(v) => setNewItem({ ...newItem, blurb: v })}/>
              </td>
              <td colSpan={2}>
          ...
          {list.values.map((v, i) => (<ValueRow key={v._id?.toString() || i} v={v} index={i} readOnly={readOnly} total={list.values.length} onUpdate={updateValue} onRemove={removeValue} onBlurbClick={setSelectedItem}/>))}
        </tbody>
      </table>
    </div>);
}
