...
import { getCategoriesMessage, getLogicMessage } from "./messages";
...
export const evaluate = async (msg: IMessage, game: HydratedDocument<IGame>, isNew: boolean) => {
  ...
  */
  ...
  switch (msg.command.toLowerCase()) {
    ...
    case "/logica":
    case "/logic":
      bot.queueMessage(msg.chat.id, getLogicMessage(game.settings.language));
      break;
    case "/comandos":
      bot.queueMessage(
        msg.chat.id,
    ...
    case "/estatisticas":
    case "/stats":
      bot.sendKeyboard(game.chat_id, `<b>${i18n(game.settings.language, "stats.stats")}</b>`, keyboards.stats());
      break;
    case "/erro":
    case "/typo":
    ...
    case "/eu":
    case "/me":
      getStats(msg.chat.id, { id: "p_" }, msg.from.id);
      break;
    case "/pontuacao":
    case "/score":
