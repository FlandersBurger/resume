    ...
    switch (type) {
      ...
      case 'global':
        ...
        .exec((err, games) => {
          ...
          message += `Hints Asked: ${allPlayers.reduce((count, player) => count + (player.hints ? player.hints : 0), 0)}\n`;
          message += `Lists Skipped: ${allPlayers.reduce((count, {skips}) => count + (skips ? skips : 0), 0)}\n`;
          message += `${allPlayers.filter(({scoreDaily}) => scoreDaily).length} out of ${allPlayers.length} players played today\n`;
      ...
      case 'g':
        List.find().exec((err, {length}) => {
          ...
          message += `Hints Asked: ${game.players.reduce((count, player) => count + player.hints, 0)}\n`;
