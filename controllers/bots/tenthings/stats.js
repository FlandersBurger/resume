...
exports.getList = (game, callback) => {
  ...
  game.list.values.forEach(({
    ...
    if (!guesser || !guesser.first_name) {
      str += `<b>${index + 1}:</b> `;
      str += `<b>${hints.getHint(game.hints, value)}</b>${value.indexOf('*') >= 0 ? ' - <i>WARNING: * in answer</i>' : ''}`;
      str += '\n';
    } else {
      str += `${index + 1}: `;
...
exports.getStats = async (chat_id, data, requestor) => {
  ...
  switch (type) {
    ...
    case 'leastpopular':
      ...
      break;
    case 'skippers':
      playerStats(game, players, 'skips', 'lists', 1, 'Most Skips Requested', 'Skip commands / Lists played', 1, data.requestor);
      break;
    case 'unskippers':
      playerStats(game, players, 'skips', 'lists', 1, 'Least Skips Requested', 'Skip commands / Lists played', -1, data.requestor);
      break;
    case 'answers':
    ...
    case 'snubs':
      ...
      break;
    case 'unsnubs':
      playerStats(game, players, 'snubs', 'answers', 1, 'Least Snubs', 'Answers that were already answered / Total answers', -1, data.requestor);
      break;
    case 'hints':
      playerStats(game, players, 'hints', 'lists', 1 / 6, 'Most Hints Asked', '(Hint commands / 6) / Lists played', 1, data.requestor);
      break;
    case 'unhints':
      playerStats(game, players, 'hints', 'lists', 1 / 6, 'Least Hints Asked', '(Hint commands / 6) / Lists played', -1, data.requestor);
      break;
    case 'plays':
      playerStats(game, players, 'plays', '', 1, 'Most Games Played', 'Sum days played', 1, data.requestor);
      break;
    case 'wins':
    ...
    case 'unwins':
      ...
      break;
    case 'astreak':
      playerStats(game, players, 'streak', '', 1, 'Best Answer Streak', 'Consecutive answers given', 1, data.requestor);
      break;
    case 'pstreak':
      playerStats(game, players, 'maxPlayStreak', '', 1, 'Best Play Streak', 'Consecutive days played', 1, data.requestor);
      break;
    case 'hstreak':
      playerStats(game, players, 'maxHintStreak', '', 1, 'No Hint Streak', 'Consecutive answers given without asking hints', 1, data.requestor);
      break;
    default:
