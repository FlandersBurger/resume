...
const resetDailyScore = (force = false) => {
	if (moment().utc().hour() === 1 || force) {
		bot.notifyAdmin(
			`Score Reset Triggered; ${moment().format('DD-MMM-YYYY hh:mm')}`
		);
		TenThingsGame.find({
			lastPlayDate: {
				...
				async games => {
					...
					for (let game of games) {
						...
						let winners = players.filter(
							player => player.scoreDaily === highScore
						);
						let message = winners.reduce(
							(msg, { first_name }, index, length) => {
								if (index < length - 1) {
									msg += `${first_name} & `;
								} else {
									msg += first_name;
						...
						stats.getList(game, list =>
							bot.queueMessage(game.chat_id, message + list)
						);
						game.save();
					}
					updateDailyStats(
		...
		bot.notifyAdmin(
			`Schedule incorrectly triggered: ${moment().format('DD-MMM-YYYY hh:mm')}`
		);
	}
};
...
const updateDailyStats = async (games, totalPlayers, uniquePlayers) => {
	...
	message += `${(
		listStats[0].plays - base.listsPlayed
	).makeReadable()} lists played\n`;
	message += `${(
		listStats[0].votes - base.votes
	).makeReadable()} list votes given\n`;
	message += `${(
		playerStats[0].skips - base.skips
	).makeReadable()} lists skipped\n`;
	message += `${(
		playerStats[0].answers - base.answers
	).makeReadable()} answers given\n`;
	message += `${(
		playerStats[0].snubs - base.snubs
	).makeReadable()} answers snubbed\n`;
	message += `${(
	...
	message += `${(
		playerStats[0].score - base.score
	).makeReadable()} points scored overall\n`;
	message += `${(
		playerStats[0].suggestions - base.suggestions
	).makeReadable()} suggestions given\n`;
	message += `${(
		playerStats[0].searches - base.searches
	).makeReadable()} lists searched\n`;
	...
	const updatePlayStreak = schedule.scheduleJob('0 0 2 * * *', () => {
		...
		TenThingsPlayer.find({
			...
			.then(players => {
				...
				if (players.length > 0)
					bot.notifyAdmin(
						`${players.length} game streaks updated in ${uniqueGames.length} games`
					);
