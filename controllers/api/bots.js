...
router.get('/lists/:id/tv', async (req, res, next) => {
  ...
  if (list) {
    let changed = false;
    for (let value of list.values) {
      const movieDB = await request(`https://api.themoviedb.org/3/search/tv?api_key=${config.tokens.tmdbapi}&query=${encodeURIComponent(value.value)}`);
      try {
        const posterPath = JSON.parse(movieDB).results[0].poster_path;
        if (posterPath) {
          value.blurb = `http://image.tmdb.org/t/p/w500${posterPath}`;
